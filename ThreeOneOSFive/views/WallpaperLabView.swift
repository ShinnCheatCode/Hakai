...
struct WallpaperLabView: View {
    ...
    private func alertContent(_ alert: WallpaperLabAlert) -> Alert {
        ...
        case .reset:
            return Alert(
                title: Text(language.text("wallpaper.reset_confirm_title")),
                message: Text(language.text("wallpaper.reset_confirm_message")),
                primaryButton: .destructive(
                    Text(language.text("wallpaper.reset")),
                    action: {
                        if case .reset(let receipts) = alert.kind {
                            resetAllWallpapers(receipts)
                        }
                    }
                ),
                secondaryButton: .cancel(
        ...
        case .deletePackage(let package):
            return Alert(
                title: Text(language.text("browser.delete")),
                message: Text(
                    language.text(
                        "wallpaper.delete_confirm_message",
                        package.displayName
                    )
                ),
                primaryButton: .destructive(
                    Text(language.text("browser.delete")),
    ...
    private func reloadLocalData() {
        packages = (try? WallpaperPackageStore.stagedPackages()) ?? []
