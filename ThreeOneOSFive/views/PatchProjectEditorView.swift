...
struct PatchProjectEditorView: View {
    ...
    private func ruleRow(_ rule: PatchRule) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            if rule.hasReplacement {
                ...
                Label(
                    language.text("patch.replacement_required"),
    ...
    private func saveCompleteProject(named projectName: String) {
        ...
        do {
            ...
            onSave(
                project,
                existingProject == nil && !password.isEmpty ? password : nil
            )
            dismiss()
        } catch let error as PatchPackageError {
            validationMessageKey = error.localizationKey
...
struct PatchRuleEditorView: View {
    ...
    init(
        rule: PatchRule?,
        onSave: @escaping (PatchRule) -> Void
    ) {
        originalRule = rule
        self.onSave = onSave
            ...
            Form {
                ...
                Section(language.text("patch.replacement_file")) {
                    Button {
                        ...
                        HStack(spacing: 10) {
                            Image(
                                systemName: replacementFilename.isEmpty
                                    ? "doc.badge.plus"
                                    : "doc.fill"
                            )
                            .foregroundStyle(AppTheme.accent)
    ...
    private func save() {
        do {
            let canonicalBundle =
                try PatchPathValidator.canonicalBundleIdentifier(
                    bundleID
                )

            let canonicalPath =
                try PatchPathValidator.canonicalRelativePath(
                    relativePath
                )

            guard !replacementFilename.isEmpty else {
                throw PatchPackageError.invalidProject
            }
